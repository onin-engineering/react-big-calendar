...
function inRange(e, start, end, accessors, timer) {
  ...
  var endsAfterStart =
    dates.startOf(start, 'minutes') <= dates.startOf(endTimestamp, 'minutes') // let eStart = dates.startOf(accessors.start(e), 'day')
...
function sortEvents(evtA, evtB, accessors) {
  var startSort =
    ...
    +dates.startOf(accessors.start(evtB), 'day') // sort by start Day first
